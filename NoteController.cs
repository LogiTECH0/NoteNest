...
public class NoteController : ControllerBase
{
    ...
    [Authorize]
    [HttpPut("boards/{boardId}/columns/{columnId}/notes/{noteId}/recolor")]
    public IActionResult RecolorNote(int boardId, int columnId, int noteId, string color, string text)
    {
        var userId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)!.Value);
        _noteService.RecolorNote(userId, boardId, columnId, noteId, color, text);
        return Ok();
    }
}